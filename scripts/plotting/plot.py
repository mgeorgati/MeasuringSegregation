...
def plotProjections(destName, city, cityDestPath, attr_value, scenario, year, districtPath, neighPath, streetsPath, waterPath, invertArea):
    srcPathTif = cityDestPath + '/data/GeoTIFF/' + destName + '.tif'
    if scenario =='hist':
        exportPath =  cityDestPath + '/data/PNG/{0}/'.format(attr_value, scenario)
    else:
        exportPath =  cityDestPath + '/data/PNG/{0}/{1}/'.format(attr_value, scenario)
